...
def pySPM_SXM(path):
    try:
        ...
        print("\nError detected for file path:", path)
        print(error)
        raise
...
def read_mtrx_v1(path):
    ...
    try:
        ...
        mtrx_channels = {"Z": glob.glob(Z_path), "Current": glob.glob(I_path)}
...
def read_mtrx(path, metadata_source):
    ...
    try:
        ...
        mtrx_channels = {"Z": glob.glob(str(Z_path)), "Current": glob.glob(str(I_path))}
        ...
        for channel, channel_paths in mtrx_channels.items():
            for channel_path in channel_paths:
                ...
                for _, raster in rasters.items():
                    ...
                    try:
                        # Retreive records of important scan parameters
                        setpoint, setpoint_unit = mtrx.param[
                            "EEPA::Regulator.Setpoint_1"
                        ]
                        bias, _ = mtrx.param["EEPA::GapVoltageControl.Voltage"]
                        width, _ = mtrx.param["EEPA::XYScanner.Width"]
                        height, _ = mtrx.param["EEPA::XYScanner.Height"]
                        relocation_step, _ = mtrx.param[
                            "EEPA::XYScanner.Relocation_Step_Limit"
                        ]
                        relocation_time, _ = mtrx.param[
                            "EEPA::XYScanner.Relocation_Time_Limit"
                        ]
                        raster_time, _ = mtrx.param["EEPA::XYScanner.Raster_Time"]
                        speed = width / raster_time
...
def read_dir(path="", filename_filter=""):
...
def read_single(path, metadata_source=None):
    if DEBUG:
        print("Reading single path...", path)
...
def read_list(paths, metadata_source=None):
    ...
    for i, file_name in enumerate(paths):
        ...
        #     n_paths,
        #     10,
        #     f"Reading {n_paths} spm images...Done"
        #     if i + 1 == n_paths
        #     else f"Reading {i}/{n_paths} sxm images...",
        # )
        spm_data = read_single(file_name, metadata_source)
        data.append(spm_data)
    # print("")
...
def read(path, filename_filter="", metadata_source=None):
    ...
    """
    # Check if path leads to a file, folder, or a list of paths
    if type(path) is list:
        data = read_list(path)
    elif os.path.isfile(path):
        data = [read_single(path)]
    else:
        # if path points to a folder, get all paths in folder according to filter
        if DEBUG:
            print("Searching Directory...")
        paths = read_dir(path, filename_filter)
...
def export_images(images, dst_paths=["./"], ext="jpg", scan_dir="up", cmap="gray"):
    """
